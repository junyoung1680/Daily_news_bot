...
for article in top_articles:
    ...
    # 💡 [프롬프트 튜닝] 특수기호/소제목 금지, 개조식(글머리 기호) 강제
    prompt = f"""
    당신은 바쁜 실무자를 위한 IT 뉴스 브리핑 전문가입니다. 
    다음 기사를 원문을 보지 않아도 완벽히 이해할 수 있도록 가독성 최고의 '개조식(글머리 기호)' 형태로 요약하세요. 
    줄글(문단)로 길게 풀어쓰는 것을 엄격히 금지합니다.

    [기사 제목]: {title}

    작성 가이드:
    1. 제목: 기사 전체를 아우르는 통찰력 있는 국문 제목을 첫 줄에 작성하세요. (단, *, # 같은 특수기호나 이모지는 절대 사용하지 말고 텍스트로만 깔끔하게 적으세요.)
    2. 소제목 금지: '[분석]', '1. 팩트' 같은 억지스러운 소제목이나 라벨을 쓰지 마세요.
    3. 본문 구조 (반드시 글머리 기호 '•' 사용):
       • (핵심 사건을 명확하게 1~2문장으로 요약)
       • (구체적인 데이터나 주요 기업 동향 기술)
       • (이 사건이 미치는 영향이나 향후 전망 기술, 이때 인과관계나 흐름을 나타내는 화살표 '→' 적극 활용)
    4. 출처나 링크 정보는 절대 포함하지 마세요.
    """
