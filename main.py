...
final_message = "🤖 *오늘의 로이터 심층 뉴스 브리핑 (퀄리티 테스트 모드)* 🤖\n\n"
...
for article in top_articles:
    ...
    # ⛔ [테스트 모드] 이미 요약했던 기사 패스 기능 비활성화 (무조건 요약함)
    # if title in history:
    #     print(f"🔄 이미 발송된 기사입니다 (패스): {title}")
    #     continue
        
    # [조건 2] 24시간이 지난 기사면 패스 (이 조건은 유지)
    ...
    # 💡 [프롬프트 튜닝 존] 아래 따옴표 세 개(""") 사이의 명령어를 자유롭게 수정해 보세요!
    prompt = f"""
